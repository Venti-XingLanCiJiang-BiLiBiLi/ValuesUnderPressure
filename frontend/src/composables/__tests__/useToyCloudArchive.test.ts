 ...
 *   - saveToCloud：latest + h<ts> 双写、摘要符合 value ≤1024B 约束；
 *   - 容量管理：占满 128 key 时先删最旧历史份再写；
 *   - listCloudArchives：按时间倒序、latest 与 h 同时间戳去重；
 *   - deleteCloudArchive：删除 h 并清理指向它的 latest；
...
describe('saveToCloud', () => {
  ...
  it('写入 latest 与 h<ts> 两份，内容为摘要 JSON', async () => {
    const fake = makeFakeToy()
    mountToy(fake)
    const { saveToCloud } = useToyCloudArchive()
    const res = await saveToCloud(makeResult({ confidence: 0.88 }))
...
describe('deleteCloudArchive', () => {
  it('删除历史份；若 latest 指向它则一并清理', async () => {
    const entry = JSON.stringify({ v: 1, t: 1000, q: 50, c: 0.9, cf: 0, dims: {} })
    const fake = makeFakeToy({ h1000: entry, latest: entry })
    mountToy(fake)
