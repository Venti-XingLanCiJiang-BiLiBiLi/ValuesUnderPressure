...
def test_reload_bank_success():
    ...
    try:
        with patch("app.routers.admin.load_bucket_bank") as mock_load:
            mock_bank = MagicMock()
            ...
            result = reload_bank(_req(), x_admin_token="test-token-123")
...
def test_reload_bank_wrong_token():
    ...
    try:
        with pytest.raises(HTTPException) as exc:
            reload_bank(_req(), x_admin_token="wrong-token")
...
def test_reload_bank_no_token_configured():
    ...
    with pytest.raises(HTTPException) as exc:
        reload_bank(_req(), x_admin_token="any-token")
    assert exc.value.status_code == 403
...
def test_reload_bank_corrupted_bank():
    ...
    try:
        with patch("app.routers.admin.load_bucket_bank", side_effect=FileNotFoundError("题库文件缺失")):
            with pytest.raises(HTTPException) as exc:
                reload_bank(_req(), x_admin_token="test-token")
